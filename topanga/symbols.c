...
kern_return_t offsets_init() {
    ...
    if (err == -1) {
        printf("[ERROR]: failed to detect version (sysctlbyname) failed\n");
        have_syms = 0;
        ret = KERN_FAILURE;
        return ret;
    }
